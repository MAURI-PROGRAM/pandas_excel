...
consulta1=pysqldf("SELECT SUBCAMPANAPORCLIENTE as fase,sum(SALDO)as valor,count(*) as asignados  FROM df group by SUBCAMPANAPORCLIENTE order by valor desc;")

consulta2=pysqldf("SELECT count(DISTINCT IDENTIFICACION_DEUDOR) as CANTIDAD,SUM(SALDO) as MONTO_TOTAL   FROM df ;")

consulta3=pysqldf("""SELECT  CASE 
...
consulta4=df[['SALDO','DIAS_MORA','CODIGO_CONDICION']].describe()
